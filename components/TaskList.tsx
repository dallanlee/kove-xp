...
export default function TaskList({ tasks, completedTaskIds: initialCompleted, date, onUpdate }: TaskListProps) {
  ...
  async function toggleTask(task: Task) {
    ...
    const endpoint = isCompleted ? '/api/uncomplete-task' : '/api/complete-task'

    try {
      const res = await fetch(endpoint, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ task_id: task.id, date }),
      })

      if (!res.ok) {
        // Revert on error
        const reverted = new Set(completedIds)
        setCompletedIds(reverted)
      } else {
        onUpdate?.()
      }
    } catch {
