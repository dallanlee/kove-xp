...
export default async function BankPage() {
  ...
  const { data: family } = await supabase
    .from('families')
    .select('dollar_balance, screen_time_minutes, xp_balance')
    .single()
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6">
      ...
      {/* XP Balance + progress bar */}
      <div className="bg-slate-800 rounded-2xl p-4 border border-slate-700 mb-6">
        <div className="flex justify-between items-center mb-2">
          <div>
            <div className="text-xs text-slate-400">Carried XP Balance</div>
        ...
        <p className="text-xs text-slate-500 mt-2">
          1,000 XP = $1 · 1,000 XP = 30 min screen time
        </p>
      </div>
                ...
                <div className="flex justify-between items-start mb-2">
                  <div>
                    ...
                    <div className="text-sm font-bold text-white">
                      {payout.total_xp.toLocaleString()} XP earned
                    </div>
                  </div>
                  <div className="text-right">
